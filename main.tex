...
Analysis of the score distribution (Figure \ref{fig:scores_distribution}) sheds some light on the nature of the problems with automatic and human answers. The automatic systems generate non-relevant answers ($score=1$) more often than human. Typical the reason is that model either fails to retrieve relevant information or distinguish between useful and non-useful answer candidates.
